...
<body class="bg-canvas text-ink">
    ...
    <header class="sticky top-0 z-40 border-b border-line/70 bg-canvas/85 backdrop-blur">
        <div class="mx-auto flex max-w-6xl items-center justify-between px-5 py-4">
            ...
            <div class="flex items-center gap-2">
                <a href="{{ route('login') }}" class="btn-ghost !py-2.5">تسجيل الدخول</a>
                <a href="{{ route('register') }}" class="btn-primary !py-2.5">ابدأ مجاناً</a>
            </div>
        </div>
    </header>
        ...
        <div class="mx-auto grid max-w-6xl items-center gap-10 px-5 py-16 lg:grid-cols-2 lg:py-24">
            <div class="animate-fade-up">
                ...
                <div class="mt-7 flex flex-wrap items-center gap-3">
                    <a href="{{ route('register') }}" class="btn-primary !px-7 !py-3.5 text-base">صمّم مساحتك الآن</a>
            ...
            <div class="relative hidden lg:block">
                <div class="overflow-hidden rounded-[2.5rem] shadow-soft">
                    <img src="{{ asset('images/rooms/living-warm.svg') }}" alt="تصميم غرفة معيشة" class="w-full">
                </div>
                <div class="absolute -bottom-6 -left-6 w-52 overflow-hidden rounded-3xl border-4 border-white shadow-card">
                    <img src="{{ asset('images/rooms/bedroom-scandi.svg') }}" alt="غرفة نوم" class="w-full">
                </div>
            ...
            <div class="mt-10 grid gap-6 sm:grid-cols-3">
                @foreach ([
                    ...
                    <div class="card p-7 text-center">
                        <span class="mx-auto grid h-12 w-12 place-items-center rounded-full bg-clay text-lg font-bold text-white">{{ $n }}</span>
                        <h3 class="mt-4 font-display text-xl">{{ $title }}</h3>
    ...
    @if ($featured->isNotEmpty())
        <section id="store" class="mx-auto max-w-6xl px-5 py-16">
            <div class="flex items-end justify-between">
                ...
                <a href="{{ route('register') }}" class="text-sm font-bold text-clay">تصفّح الكل ←</a>
            </div>
                ...
                @foreach ($featured as $product)
                    <div class="card group overflow-hidden">
                        ...
                        <div class="flex items-center justify-between p-4">
                            ...
                            <span class="font-bold text-clay">{{ $product->priceLabel() }}</span>
                        </div>
                    </div>
                @endforeach
            </div>
            ...
            <div class="mx-auto max-w-6xl px-5 py-16">
                ...
                <div class="mt-8 grid gap-6 sm:grid-cols-3">
                    @foreach ($inspirations as $insp)
                        <div class="group relative h-56 overflow-hidden rounded-3xl" style="background:{{ $insp->accent_color }};">
                            @if ($insp->image_path)
    ...
    <section class="mx-auto max-w-6xl px-5 py-16">
        <div class="relative overflow-hidden rounded-[2.5rem] bg-ink p-10 text-center text-white sm:p-14">
            ...
            <a href="{{ route('register') }}" class="btn-primary relative mt-7 !px-8 !py-3.5 text-base">ابدأ الآن — مجاناً</a>
        </div>
    </section>
    ...
    <footer class="border-t border-line/70">
        <div class="mx-auto flex max-w-6xl flex-wrap items-center justify-between gap-4 px-5 py-8 text-sm text-muted">
            @include('partials.brand')
            <p>© {{ date('Y') }} سارا هوم — مشروع تخرّج بإشراف أكاديمي.</p>
